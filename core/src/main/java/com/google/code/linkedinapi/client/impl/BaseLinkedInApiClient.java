...
public abstract class BaseLinkedInApiClient implements LinkedInApiClient {
	...
	private String testUrl = "undefined";
    ...
    public void sendInviteByEmail(String email, String firstName, String lastName, String subject, String message) {
        ...
        RecipientImpl recepient = (RecipientImpl) OBJECT_FACTORY.createRecipient();
	...
	public CompanyStatistics getCompanyStatistics(String id) {
        ...
        LinkedInApiUrlBuilder builder = createLinkedInApiUrlBuilder(LinkedInApiUrls.GET_COMPANY_STATISTICS);
        String                apiUrl  = builder.withEmptyField(ParameterNames.FIELD_SELECTORS).withField(ParameterNames.ID,
        									id).buildUrl();
	...
	@Override
	public Posts getMyGroupPosts(String groupId, Set<PostField> postFields, String roleType) {
        LinkedInApiUrlBuilder builder = createLinkedInApiUrlBuilder(LinkedInApiUrls.GET_GROUP_MEMBERSHIP_ROLE);
        String                apiUrl  = builder.withFieldEnumSet(ParameterNames.FIELD_SELECTORS, postFields)
        		.withField(ParameterNames.ID, groupId)
        		.withParameter(ParameterNames.ROLE, roleType).buildUrl();
        testUrl = apiUrl;
        return readResponse(Posts.class, callApiMethod(apiUrl));
	}
	
	@Override
	public Posts getMyGroupPosts(String groupId, Set<PostField> postFields, String roleType, int start, int count) {
        LinkedInApiUrlBuilder builder = createLinkedInApiUrlBuilder(LinkedInApiUrls.GET_GROUP_MEMBERSHIP_ROLE);
        String                apiUrl  = builder.withFieldEnumSet(ParameterNames.FIELD_SELECTORS, postFields)
        		.withField(ParameterNames.ID, groupId)
        		.withParameter(ParameterNames.ROLE, roleType)
        		.withParameter(ParameterNames.START, String.valueOf(start)).withParameter(ParameterNames.COUNT, String.valueOf(count)).buildUrl();
	...
	public GroupMemberships getGroupMemberships(
		...
		// Remove Posts field 
		groupMembershipFields.remove(GroupMembershipField.POSTS);
        LinkedInApiUrlBuilder builder = createLinkedInApiUrlBuilder(LinkedInApiUrls.GET_GROUP_MEMBERSHIPS);
    ...
    protected LinkedInApiClientException createLinkedInApiClientException(Error error) {
        final String message    = error.getMessage() + " " + testUrl;
        final String errorCode  = error.getErrorCode() + " " + testUrl;
        final String requestId  = error.getRequestId() + " " + testUrl;
